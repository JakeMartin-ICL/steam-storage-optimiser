...
def load_config():
    try:
        ...
        with open("config.json", 'w') as f:
            json.dump(config, f)
        input(f"{Fore.GREEN}Saved new config file.{Style.RESET_ALL} Press any key to continue.")
    return config
...
def get_library_paths(config):
    try:
        ...
        print(f"{Fore.RED}Problem with libraryfolders file.{Style.RESET_ALL} If Steam not installed at {Fore.YELLOW}{config['install_dir']}{Style.RESET_ALL}, edit your config file and try again")
        os.system("pause")
        sys.exit(1)
...
def match_games(config, api_dict):
    ...
    for library_path in get_library_paths(config):
        ...
        print(f"{Fore.GREEN}Found {len(manifests)} in library {library_path}{Style.RESET_ALL}")
            ...
            try:
                ...
                for game in manifest.values():
                    ...
                    try:
                        ...
                        print(f"{Style.DIM}Failed to match {appid}: {game['name']}.{Style.RESET_ALL}")
                    else:
